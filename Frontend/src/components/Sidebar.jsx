...
function Sidebar({ open }) {
  ...
  return (
    <div
      className={`bg-slate-100 min-h-screen fixed transition-transform ${open ? "w-60" : "w-16"} duration-500 rounded-lg shadow-md`}
    >
      <span className="mt-24 flex flex-col gap-4 mx-2 relative">
        {menus?.map((menu, i) => (
          <Link
            ...
            className={` ${
              menu?.margin && "mt-5"
            } group flex items-center text-md gap-10 font-semibold p-3 hover:bg-gray-200 rounded-md text-gray-900`}
          >
            ...
            <h1
              className={`${open && "hidden"} absolute left-48 bg-white font-normal whitespace-pre text-gray-900 rounded-md drop-shadow-lg px-0 py-0 w-0 
